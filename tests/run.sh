...
test_zellij_plugin_generates_current_theme() {
  ...
  local hook_dir="$home_dir/.config/omarchy/hooks/theme-set.d"
  local theme_file="$home_dir/.config/zellij/themes/current.kdl"
  ...
  assert_file_exists "$theme_file" "zellij plugin writes current theme"
  assert_contains "$(cat "$theme_file")" "Generated by thpm zellij plugin" "zellij theme identifies generated owner"
  assert_contains "$(cat "$theme_file")" "background 16 17 18" "zellij theme uses primary background"
  assert_contains "$(cat "$theme_file")" "base 241 242 243" "zellij theme uses primary foreground"
  assert_contains "$(cat "$theme_file")" "table_title {
            base 85 85 85" "zellij table title uses normal magenta"
  assert_contains "$(cat "$theme_file")" "background 34 34 34" "zellij theme uses selection background"
  assert_eq 'theme "old"' "$(cat "$config_file")" "zellij plugin does not edit main config"
}
...
EOF
  ...
  assert_file_exists "$theme_file" "zellij plugin writes provided current theme"
  assert_contains "$(cat "$theme_file")" "Generated by thpm zellij plugin" "zellij provided theme keeps ownership header"
  assert_contains "$(cat "$theme_file")" "base 1 2 3" "zellij plugin copies theme-provided kdl"
  assert_contains "$(cat "$theme_file")" "background 4 5 6" "zellij plugin preserves theme-provided colors"
}
...
test_zellij_plugin_skips_when_zellij_missing() {
  ...
  assert_file_missing "$theme_file" "zellij plugin skips without zellij command"
}
